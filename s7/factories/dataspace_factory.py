...
def __class_factory(name, meta):
    ...
    gen_getset = lambda key: \
        lambda: property(
            lambda self: self.properties[key]["value"] if (
                "value" in self.properties[key]) else None,
            lambda self, value: self.properties[key].update({"value": value}),
            doc=meta['properties'][key]["description"] if (
                "description" in meta['properties'][key]) else None)
...
def __dataspace_class_factory(name, meta, dataspace):
    ...
    initializer = (
        lambda __db:
            lambda self,
                id=None: self.__dict__.update({'id': (__db.document(id)).id}))(db)
    generic_setter = (
        lambda db:
            lambda self, key, value:
                (db.document(self.id).update({key: value})))(db)
    attr = dict(
        uriref = meta['uri'],
    ...
    gen_dataspace_getset = \
        lambda db: \
            lambda key: \
                lambda: property(
                    lambda self: db.document(self.id).data[key] if (
                        key in db.document(self.id).data) else None,
