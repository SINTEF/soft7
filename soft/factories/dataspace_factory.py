...
def __dataspace_class_factory(name, meta, dataspace):
    ...
    initializer = (
        lambda __db: lambda self, id=None: self.__dict__.update(
            {"id": (__db.document(id)).id}
        )
    )(db)
    generic_setter = (
    ...
    gen_dataspace_getset = lambda db: lambda key: lambda: property(
        lambda self: db.document(self.id).data[key]
        if key in db.document(self.id).data
        else None,
